...
var words = ["Welcome", "hello", "other"]
// var words = ["Welcome", "hello", "what is", "the weather", "the time",
  ...
  createPredictBtn(){
    ...
    predButton.addEventListener('mousedown', () => {
      ...
      if(Math.max(...exampleCount) > 0){
        ...
        if(exampleCount[0] == 0){
          alert(
            `You haven't added examples for the wake word Welcome`
            )
          return
        }
...
class TextToSpeech{
  ...
  speak(word){
    ...
    if(word == 'Welcome'){
      ...
      setTimeout(() => {
        // if no query detected after Welcome is signed
    ...
    if(word != 'Welcome' && this.currentPredictedWords.length == 0){
      console.log("first word should be Welcome")
      console.log(word)
      return
    }
